...
export function analyzeDiffForCommit(diff: string, status: GitRepoStatus): CommitSuggestion {
...
export function performPrReview(diff: string, status: GitRepoStatus): PrReviewReport {
  ...
  let verdict: PrReviewReport["verdict"] = "APPROVED";
  if (criticalCount > 0) {
...
export function generateReleaseChangelog(commits: Array<{ hash: string; author: string; date: string; message: string }>): string {
  ...
  for (const c of commits) {
    ...
    if (/^feat/i.test(msg)) categories["✨ Features"].push(entry);
    else if (/^fix/i.test(msg)) categories["🐛 Bug Fixes"].push(entry);
    else if (/^perf/i.test(msg)) categories["⚡ Performance"].push(entry);
    else if (/^refactor/i.test(msg)) categories["♻️ Refactoring"].push(entry);
    else if (/^docs/i.test(msg)) categories["📝 Documentation"].push(entry);
    else categories["🔧 Maintenance"].push(entry);
  }
...
export function suggestConflictResolution(conflict: GitConflictBlock): { strategy: string; resolution: string; explanation: string } {
  return {
